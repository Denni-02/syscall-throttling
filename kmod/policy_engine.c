...
void enforce_syscall_policy(int syscall_num) {
    ...
    if (is_monitored) {
        ...
        /* Il thread rimarrà dormiente finché un'altra entità non incrementerà time_epoch
         * e chiamerà wake_up_all() su throttle_queue.
         */
        wait_event_interruptible(throttle_queue, atomic_read(&time_epoch) > sleep_epoch);
        
        printk(KERN_INFO "[Syscall_Throttling] RISVEGLIO! Processo %s riprende l'esecuzione nell'epoca %d.\n", 
               caller_comm, atomic_read(&time_epoch));
    } else {
